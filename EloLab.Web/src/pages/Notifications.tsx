...
export function Notifications() {
    ...
    useEffect(() => {
        ...
        const handleNewRealtimeNotif = (e: any) => {
            const nova = e.detail;
            setNotificacoes(prev => [nova, ...prev]);
        };

        window.addEventListener('elolab_nova_notificacao', handleNewRealtimeNotif);
        return () => window.removeEventListener('elolab_nova_notificacao', handleNewRealtimeNotif);
    }, []);
    ...
    async function handleCardClick(notif: any) {
        if (!notif.lida) {
            try {
                ...
                setNotificacoes(prev => prev.map(n => n.id === notif.id ? { ...n, lida: true, Lida: true } : n));
                // Avisa a Sidebar que uma notificação foi lida
                window.dispatchEvent(new CustomEvent('elolab_notificacoes_atualizar'));
            } catch (e) { console.error(e); }
        }
        if (notif.linkAction) navigate(notif.linkAction);
    }
    ...
    function openDeleteAll() {
        setModalConfig({
            ...
            type: 'delete-all',
            title: 'Limpar tudo?',
            message: 'Tem a certeza que deseja apagar todas as notificações?',
    ...
    async function handleConfirmAction() {
        try {
            if (modalConfig.type === 'delete-one' && modalConfig.targetId) {
                ...
                setNotificacoes(prev => prev.map(n => ({ ...n, lida: true, Lida: true })));
                notify.success("Tudo marcado como lido.");
                window.dispatchEvent(new CustomEvent('elolab_notificacoes_atualizar'));
            }
        } catch (error) {
            notify.error("Erro ao processar a ação.");
        } finally {
    ...
    const naoLidas = notificacoes.filter(n => !n.lida && !n.Lida).length;
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setModalConfig({ ...modalConfig, isOpen: false })} className="flex-1 rounded-xl bg-slate-100 py-3 font-bold text-slate-600 hover:bg-slate-200 transition">Cancelar</button>
                            <button onClick={handleConfirmAction} className={`flex-1 rounded-xl py-3 font-bold text-white shadow-lg transition ${modalConfig.isDestructive ? 'bg-red-600 hover:bg-red-700' : ''}`} style={!modalConfig.isDestructive ? { backgroundColor: primaryColor } : {}}>
            ...
            <div className="mx-auto max-w-4xl">
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Acompanhe as atualizações em tempo real.</p>
                    </div>

                    <div className="flex gap-2">
                        {naoLidas > 0 && (
                            <button onClick={openMarkAllRead} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold bg-white border border-slate-200 shadow-sm text-slate-600 hover:bg-slate-50 transition">
                                <Check className="h-4 w-4" /> <span className="hidden sm:inline">Marcar lidas</span>
                            </button>
                        )}
                        {notificacoes.length > 0 && (
                            <button onClick={openDeleteAll} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold bg-white border border-slate-200 shadow-sm text-red-500 hover:bg-red-50 transition">
                        ...
                        notificacoes.map((n) => (
                            <div
                                ...
                                className={`group relative overflow-hidden rounded-2xl border p-4 transition-all cursor-pointer hover:shadow-md
                                    ${(!n.lida && !n.Lida) ? 'bg-white border-blue-200 shadow-sm' : 'bg-slate-50 border-slate-200 opacity-85 hover:opacity-100'}`}
                            >
                                {(!n.lida && !n.Lida) && <div className="absolute top-0 left-0 w-1.5 h-full" style={{ backgroundColor: primaryColor }}></div>}
                                <div className="flex items-center gap-4">
                                    <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${(!n.lida && !n.Lida) ? 'bg-blue-50' : 'bg-white border border-slate-200'}`}>
                                        {getIcon(n.titulo)}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-1">
                                            <h3 className={`text-base truncate pr-2 ${(!n.lida && !n.Lida) ? 'font-bold text-slate-900' : 'font-semibold text-slate-700'}`}>{n.titulo}</h3>
                                            <span className="text-xs font-medium text-slate-400 whitespace-nowrap">
                                                {new Date(n.createdAt).toLocaleDateString()} • {new Date(n.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                            </span>
                                        </div>
                                        <p className="text-sm text-slate-600 leading-relaxed line-clamp-2">{n.texto}</p>
                                    </div>
                                    <div className="flex items-center gap-2 pl-3 border-l border-transparent group-hover:border-slate-100">
                                        <button onClick={(e) => openDeleteOne(n.id, e)} className="p-2.5 rounded-xl text-slate-300 hover:text-red-600 hover:bg-red-50 transition"><Trash2 className="h-5 w-5" /></button>
                                        <div className="hidden sm:block text-slate-300 group-hover:translate-x-1" style={{ color: (!n.lida && !n.Lida) ? primaryColor : undefined }}><ArrowRight className="h-5 w-5" /></div>
                                    </div>
                                </div>
                            </div>
                        ))
