...
import {
    ...
    MapPin, FileText, Loader2, X, Save
} from 'lucide-react';
...
export function Clinics() {
    ...
    async function loadData() {
        try {
            ...
            if (userData.tipo === 'Laboratorio') {
                ...
                // Voltamos a usar o endpoint geral que sempre funcionou!
    ...
    async function handleAddClinica(e: React.FormEvent) {
        ...
        try {
            ...
            // O interceptor do api.ts já vai mostrar o erro vermelho
        } finally {
            setSaving(false);
        }
    }
        ...
        <PageContainer primaryColor={primaryColor}>
            ...
            <div className="space-y-6">
                <div className="relative max-w-md">
                    ...
                    <input
                        type="text"
                        placeholder="Pesquisar clínica por nome ou NIF..."
                        value={busca}
                        onChange={e => setBusca(e.target.value)}
                        className="w-full rounded-2xl border border-slate-200 bg-white py-3.5 pl-12 pr-4 text-sm font-medium outline-none transition shadow-sm"
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <table className="w-full text-left text-sm text-slate-600">
                        ...
                        <tr>
                            ...
                            <th className="px-6 py-4">Localização / NIF</th>
                        ...
                        {clinicasFiltradas.map((c) => (
                            <tr key={c.id} className="hover:bg-slate-50 transition group">
                                ...
                                <td className="px-6 py-4 space-y-1">
                                    ...
                                    <div className="flex items-center gap-2"><MapPin className="h-3 w-3 text-slate-400"/> <span className="truncate max-w-[200px] block">{c.endereco || <span className="text-slate-300 italic text-xs">Sem Morada</span>}</span></div>
                                </td>
                                <td className="px-6 py-4">
                                    {c.usuarioId ? (
